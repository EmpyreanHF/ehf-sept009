...
#cs-file-preview.cs-media-failed,#status-file-preview.cs-media-failed{background:linear-gradient(135deg,#7F1D1D,#450A0A);animation:none;}
...
`;
    ...
    function _wireCreateModal(){
        ...
        if(cancelBtn&&!cancelBtn._v4){cancelBtn._v4=true;
            cancelBtn.addEventListener('click',function(){modal.style.display='none';modal.classList.remove('show');document.body.classList.remove('modal-open');window._empPendingRemoteStatusMedia=null;});
        }
        ...
        fileInp.addEventListener('change',function(){
            ...
            var rm=document.createElement('button');rm.type='button';rm.className='cs-rm-btn';rm.innerHTML='<i class="fas fa-times"></i>';
            rm.addEventListener('click',function(e2){e2.preventDefault();e2.stopPropagation();_clearSettleTimer();fileInp.value='';prev.innerHTML='';prev.style.display='none';prev.classList.remove('cs-loading','cs-media-failed');try{URL.revokeObjectURL(burl);}catch(ex){}});
            prev.appendChild(rm);
                ...
                try{
                    ...
                    if(window.fbDb){
                        await _writeDocWithRetry(function () {
                            return window.fbDb.collection('statuses').doc(docId).set({userId:doc.userId,name:doc.name,avatar:doc.avatar,items:mergedItems,createdAt:doc.createdAt},{merge:true});
                        });
                    }
